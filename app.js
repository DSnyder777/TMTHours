...
let nbCrewIds = [];
let nbSearch = "";
/** Per-person hour overrides: employeeId -> { estimatedHours, nonBillableHours } */
let personHourOverrides = {};
/** People whose per-person hours were edited manually */
let personHoursTouched = new Set();
let appBooted = false;
...
const els = {
  ...
  truckCrewSection: document.getElementById("truck-crew-section"),
  nbCrewSection: document.getElementById("nb-crew-section"),
  nbCrewSearch: document.getElementById("nb-crew-search"),
  nbCrewPicker: document.getElementById("nb-crew-picker"),
  logCrewCopy: document.getElementById("log-crew-copy"),
  ...
  personHoursPanel: document.getElementById("person-hours-panel"),
  ...
  loginImportInput: document.getElementById("login-import-input"),
};
...
async function init() {
  ...
  if (els.loginImportInput) {
    els.loginImportInput.addEventListener("change", onPublishImport);
  }
...
async function loadPublishedData() {
  const localIsEmpty = !state.employees.length && !state.entries.length;
  try {
    ...
    if (!response.ok) {
      if (localIsEmpty) {
        setSyncStatus(
          "local",
          "No shop data loaded — use Import data.json below (needed for offline file open)"
        );
      } else {
        refreshPublishStatus();
      }
    ...
    const remoteHasPeople =
      Array.isArray(payload.employees) && payload.employees.length > 0;
    const remoteHasJobs =
      Array.isArray(payload.entries) && payload.entries.length > 0;
    const remoteHasData = remoteHasPeople || remoteHasJobs;
    if (!remoteHasData) {
    ...
    // Prefer published file when this browser is empty, even if a stale stamp exists
    if (localIsEmpty) {
      applyShopPayload(payload, { fromPublishFile: true });
      return;
    }
    const localEntryCount = state.entries.length;
    const remoteEntryCount = Array.isArray(payload.entries)
      ? payload.entries.length
      : 0;
    // If published file has jobs and this browser has fewer, take published
    // (avoids a stale empty localStorage stamp blocking an upload)
    if (remoteEntryCount > localEntryCount) {
      applyShopPayload(payload, { fromPublishFile: true });
      return;
    }
    ...
    // Opening as a local file:// page often can't fetch data.json — use Import instead
    if (localIsEmpty) {
      setSyncStatus(
        "local",
        "Offline file open can’t auto-load data.json — use Import data.json below"
      );
    } else {
      refreshPublishStatus();
    }
...
function onPublishImport(event) {
  ...
  reader.onload = () => {
    try {
      ...
      if (!hasData && !payload.settings) {
        showToast("That file doesn’t look like shop data");
        return;
      }
      if (!applyShopPayload(payload, { fromPublishFile: true })) {
        showToast("That file doesn’t look like shop data");
        return;
      }
      showToast(
        `Imported ${state.employees.length} people · ${state.entries.length} hour entries`
      );
      if (!isLoggedIn()) {
        setSyncStatus(
          "live",
          `Imported — ${state.employees.length} people, ${state.entries.length} entries. Log in.`
        );
      }
    } catch {
...
function bootApp() {
  ...
  els.jobType.addEventListener("change", () => {
    syncJobTypeFields();
    personHoursTouched.clear();
    seedPersonHoursFromDefaults();
    renderTruckPanels();
    renderPersonHoursPanel();
  });
  ...
  els.estimate.addEventListener("input", () => {
    seedPersonHoursFromDefaults();
    renderTruckPanels();
    renderPersonHoursPanel();
  });
  els.nbHours.addEventListener("input", () => {
    seedPersonHoursFromDefaults();
    renderTruckPanels();
    renderPersonHoursPanel();
  });
  ...
  if (els.personHoursPanel) {
    els.personHoursPanel.addEventListener("input", onPersonHoursInput);
  }
  ...
  if (els.nbCrewPicker) {
    els.nbCrewPicker.addEventListener("change", onNbCrewChange);
  }
  if (els.nbCrewSearch) {
    els.nbCrewSearch.addEventListener("input", () => {
      nbSearch = els.nbCrewSearch.value;
      renderNbCrewPanel();
    });
  }
...
function syncJobTypeFields() {
  ...
  if (els.truckCrewSection) els.truckCrewSection.hidden = isNbOnly;
  if (els.nbCrewSection) els.nbCrewSection.hidden = !isNbOnly;
  if (els.logCrewCopy) {
    els.logCrewCopy.textContent = isNbOnly
      ? "Non-billable only — select anyone who gets the hours. No driver or truck required."
      : "Pick trucks first — each truck needs 1 driver and 1–4 movers. Then set Local, LD, or Non-billable only.";
  }
...
function onTruckPanelChange(event) {
  ...
  if (moverCheck) {
    ...
    if (moverCheck.checked) {
      ...
      personHoursTouched.delete(employeeId);
    }
...
function getCrewAssignments() {
  if (els.jobType.value === "nb") {
    return nbCrewIds.map((employeeId) => ({
      employeeId,
      truckNumber: null,
      jobRole: employeeRole(employeeId) === "driver" ? "driver" : "mover",
    }));
  }
  const assignments = [];
...
function updateCrewPickedCount() {
  ...
  if (els.jobType.value === "nb") {
    els.crewPickedCount.textContent = `${count} selected · anyone on roster`;
    return;
  }
...
function validateTruckCrew() {
  if (els.jobType.value === "nb") {
    if (!nbCrewIds.length) {
      return "Select at least one person for non-billable hours";
    }
    return null;
  }
...
function onNbCrewChange(event) {
  const check = event.target.closest("[data-nb-crew-id]");
  if (!check) return;
  const employeeId = check.getAttribute("data-nb-crew-id");
  if (check.checked) {
    if (!nbCrewIds.includes(employeeId)) nbCrewIds.push(employeeId);
  } else {
    nbCrewIds = nbCrewIds.filter((id) => id !== employeeId);
    personHoursTouched.delete(employeeId);
  }
  seedPersonHoursFromDefaults();
  renderNbCrewPanel();
...
function defaultJobEstimatedHours() {
  if (els.jobType.value !== "local") return 0;
  return Number(els.estimate.value) || 0;
}

function defaultJobNonBillableHours() {
  const type = els.jobType.value;
  const includeNb = els.nbToggle.checked || type === "nb";
  if (!includeNb) return 0;
  return Number(els.nbHours.value) || 0;
}

function syncPersonHourOverridesToCrew() {
  const ids = new Set(getSelectedCrewIds());
  Object.keys(personHourOverrides).forEach((id) => {
    if (!ids.has(id)) {
      delete personHourOverrides[id];
      personHoursTouched.delete(id);
    }
  });
  ids.forEach((id) => {
    if (!personHourOverrides[id]) {
      personHourOverrides[id] = {
        estimatedHours: defaultJobEstimatedHours(),
        nonBillableHours: defaultJobNonBillableHours(),
      };
    }
  });
}

function seedPersonHoursFromDefaults() {
  syncPersonHourOverridesToCrew();
  const est = defaultJobEstimatedHours();
  const nb = defaultJobNonBillableHours();
  getSelectedCrewIds().forEach((id) => {
    if (personHoursTouched.has(id) && personHourOverrides[id]) return;
    personHourOverrides[id] = {
      estimatedHours: est,
      nonBillableHours: nb,
    };
...
function readPersonHoursFromDom() {
  if (!els.personHoursPanel) return;
  els.personHoursPanel.querySelectorAll("[data-person-id]").forEach((row) => {
    const id = row.getAttribute("data-person-id");
    if (!id) return;
    const estInput = row.querySelector("[data-person-est]");
    const nbInput = row.querySelector("[data-person-nb]");
    personHourOverrides[id] = {
      estimatedHours: estInput ? Number(estInput.value) || 0 : 0,
      nonBillableHours: nbInput ? Number(nbInput.value) || 0 : 0,
    };
  });
}

function onPersonHoursInput(event) {
  const row = event.target.closest("[data-person-id]");
  if (row?.getAttribute("data-person-id")) {
    personHoursTouched.add(row.getAttribute("data-person-id"));
  }
  readPersonHoursFromDom();
  renderTruckPanels();
}

function personHoursFor(employeeId) {
  const ov = personHourOverrides[employeeId];
  if (ov) {
    return {
      estimatedHours: Number(ov.estimatedHours) || 0,
      nonBillableHours: Number(ov.nonBillableHours) || 0,
    };
  }
  return {
    estimatedHours: defaultJobEstimatedHours(),
...
function renderPersonHoursPanel() {
  if (!els.personHoursPanel) return;
  syncPersonHourOverridesToCrew();
  const assignments = getCrewAssignments();
  const type = els.jobType.value;
  const showEst = type === "local";
  const showNb = type === "nb" || (type !== "nb" && els.nbToggle.checked);
  const showPanel = assignments.length > 0 && (showEst || showNb);

  if (!showPanel) {
    els.personHoursPanel.hidden = true;
    els.personHoursPanel.innerHTML = "";
    return;
  }

  const rows = assignments
    .map((a) => {
      const hours = personHoursFor(a.employeeId);
      const role = a.jobRole === "driver" ? "driver" : "mover";
      const truckBit =
        a.truckNumber != null
          ? ` · Truck ${a.truckNumber}`
          : "";
      return `
        <div class="person-hours-row" data-person-id="${a.employeeId}">
          <div class="person-hours-name">
            ${escapeHtml(employeeName(a.employeeId))}
            <span class="role-badge role-${role}">${roleLabel(role)}</span>
            <span class="person-hours-meta">${truckBit}</span>
          </div>
          <div class="person-hours-fields">
            ${
              showEst
                ? `<label>Est hrs
                    <input type="number" min="0" step="0.25" data-person-est
                      value="${Number(hours.estimatedHours) || 0}" />
                  </label>`
                : ""
            }
            ${
              showNb
                ? `<label>NB hrs
                    <input type="number" min="0" step="0.25" data-person-nb
                      value="${Number(hours.nonBillableHours) || 0}" />
                  </label>`
                : ""
            }
          </div>
  ...
  els.personHoursPanel.hidden = false;
  els.personHoursPanel.innerHTML = `
    <div class="section-head" style="margin-bottom: 0.65rem">
      <h2 style="font-size: 1.05rem">Hours per person</h2>
      <p>Defaults match the job hours above. Change anyone who worked a different amount (e.g. showed up late).</p>
    </div>
    <div class="person-hours-list">${rows}</div>
  `;
}
...
function onLogSubmit(event) {
  ...
  if (includeNb && (!Number.isFinite(nonBillableHours) || nonBillableHours <= 0)) {
    // Allow job-level zero if every person has their own NB? Still require job-level or any person > 0
    const anyPersonNb = assignments.some(
      (a) => personHoursFor(a.employeeId).nonBillableHours > 0
    );
    if (!anyPersonNb) {
      showToast(
        isNbOnly
  ...
  if (type === "local") {
    const bad = assignments.find((a) => {
      const h = personHoursFor(a.employeeId).estimatedHours;
      return !Number.isFinite(h) || h < 0;
    });
    if (bad) {
      showToast(`Enter hours for ${employeeName(bad.employeeId)}`);
      return;
    }
    const anyEst = assignments.some(
      (a) => personHoursFor(a.employeeId).estimatedHours > 0
    );
    if (!anyEst) {
      showToast("At least one person needs estimated hours");
      return;
    }
  }

  if (isNbOnly) {
    const anyNb = assignments.some(
      (a) => personHoursFor(a.employeeId).nonBillableHours > 0
    );
    if (!anyNb) {
      showToast("Enter non-billable hours for at least one person");
      return;
    }
  }
  ...
  const sharedBase = {
    ...
    truckCount: isNbOnly ? 0 : truckCount,
    createdAt,
  ...
  assignments.forEach((assignment) => {
    const hours = personHoursFor(assignment.employeeId);
    const personEst = isLd || isNbOnly ? 0 : hours.estimatedHours;
    const personNb = includeNb ? hours.nonBillableHours : 0;
    state.entries.push({
      ...
      ...sharedBase,
      estimatedHours: personEst,
      nonBillableHours: personNb,
    });
  });
  ...
  showToast(
    isEditing
      ? `${label} updated`
      : isNbOnly
        ? `${label} saved · ${assignments.length} people`
...
function clearLogForm({ keepEdit = false } = {}) {
  ...
  nbSearch = "";
  personHourOverrides = {};
  personHoursTouched = new Set();
  if (els.nbCrewSearch) els.nbCrewSearch.value = "";
  if (!keepEdit) {
...
function startEditJob(jobKey) {
  ...
  personHoursTouched = new Set();
  members.forEach((m) => {
    personHourOverrides[m.employeeId] = {
      estimatedHours: Number(m.estimatedHours) || 0,
      nonBillableHours: Number(m.nonBillableHours) || 0,
    };
    personHoursTouched.add(m.employeeId);
  });
  ...
  if (isNbOnlyEntry(entry)) {
    ...
    els.jobType.value = "local";
    const estValues = members.map((m) => Number(m.estimatedHours) || 0);
    els.estimate.value = Math.max(...estValues, 0);
    els.endDate.value = "";
  }
  els.jobNote.value = entry.jobNote || "";

  const nbValues = members.map((m) => Number(m.nonBillableHours) || 0);
  const maxNb = Math.max(...nbValues, 0);
  const hasNb = maxNb > 0 || isNbOnlyEntry(entry);
  els.nbToggle.checked = hasNb;
  els.nbFields.hidden = !hasNb;
  els.nbHours.value = hasNb ? maxNb : "";
...
function applyMembersToTruckCrew(members) {
  if (members[0] && isNbOnlyEntry(members[0])) {
    nbCrewIds = [...new Set(members.map((m) => m.employeeId).filter(Boolean))];
    nbSearch = "";
    if (els.nbCrewSearch) els.nbCrewSearch.value = "";
    truckCount = 1;
    truckCrew = [emptyTruck()];
    ...
    .map((truck, index) => {
      ...
      const driverOptions = drivers
        .map((employee) => {
          ...
          const status = employeeHoursStatus(employee.id);
      ...
      const driverStatus = truck.driverId ? employeeHoursStatus(truck.driverId) : null;
      ...
      return `
        <div class="truck-panel" data-truck-index="${index}">
          ...
          <div class="field-row">
            ...
            <select id="truck-driver-${index}" class="${driverStatus?.isHot ? "is-over-hours" : ""}" data-truck-driver="${index}">
...
function renderNbCrewPanel() {
  if (!els.nbCrewPicker) return;
  if (els.nbCrewSearch && els.nbCrewSearch.value !== nbSearch) {
    els.nbCrewSearch.value = nbSearch;
  }

  if (!state.employees.length) {
    els.nbCrewPicker.innerHTML = `<p class="empty">No crew yet — add people on the Crew tab.</p>`;
    updateCrewPickedCount();
    return;
  }

  const query = (nbSearch || "").trim().toLowerCase();
  const people = [...state.employees].sort((a, b) => a.name.localeCompare(b.name));
  const visible = people.filter(
    (employee) => !query || employee.name.toLowerCase().includes(query)
  );

  els.nbCrewPicker.innerHTML = visible.length
    ? visible
        .map((employee) => {
          const checked = nbCrewIds.includes(employee.id);
          const status = employeeHoursStatus(employee.id);
          const hotClass = status.isHot ? " is-over-hours" : "";
          const role = employee.role === "driver" ? "driver" : "mover";
          return `
            <label class="crew-pick${checked ? " is-checked" : ""}${hotClass}">
              <input
                type="checkbox"
                data-nb-crew-id="${employee.id}"
                ${checked ? "checked" : ""}
              />
              <span class="crew-pick-name">${escapeHtml(employee.name)}</span>
              <span class="hours-pill${status.isHot ? " is-hot" : ""}">${formatHours(status.current)} hrs</span>
        ...
        .join("")
    : `<p class="empty crew-picker-empty">${
        people.length ? "No names match that search." : "No crew on roster."
      }</p>`;
...
function pendingHoursForEmployee(employeeId) {
  try {
    const type = els.jobType?.value || "local";
    const hours = personHoursFor(employeeId);
    let total = 0;
    if (type === "local") total += hours.estimatedHours;
    if (els.nbToggle?.checked || type === "nb") total += hours.nonBillableHours;
    return total;
  } catch {
    return pendingJobHours();
  }
}

function employeeHoursStatus(employeeId, pendingHours) {
  try {
    const pending =
      pendingHours == null ? pendingHoursForEmployee(employeeId) : pendingHours;
    const current = employeePayWeekHours(employeeId);
    const projected = current + (Number(pending) || 0);
...
function renderWeek() {
  ...
  const otherWeekHint = weekEmptyButOtherWeeksHaveJobsHint(weekStart, weekEnd);
  ...
  const jumpBtn = els.weekSummary.querySelector("[data-jump-week]");
  if (jumpBtn) {
    jumpBtn.addEventListener("click", () => {
      const key = jumpBtn.getAttribute("data-jump-week");
      if (!key) return;
      weekAnchor = startOfWeek(parseInputDate(key));
      weekDetailDate = "all";
      renderWeek();
    });
  }
...
function weekEmptyButOtherWeeksHaveJobsHint(weekStart, weekEnd) {
  if (weekEntriesHaveHours(weekStart, weekEnd)) return "";
  const scopeEntries = state.entries.filter(
    (e) => !isEmployeeUser() || e.employeeId === currentEmployeeId()
  );
  if (!scopeEntries.length) return "";

  let latestKey = null;
  scopeEntries.forEach((entry) => {
    weeksTouchedByEntry(entry).forEach((key) => {
      if (!latestKey || key > latestKey) latestKey = key;
    });
  });
  if (!latestKey) return "";
  const labelStart = parseInputDate(latestKey);
  const labelEnd = endOfWeek(labelStart);
  return `
    <div class="week-other-hint">
      <p>No hours in <strong>this</strong> pay week — your uploaded jobs are in other weeks (zeros below are normal).</p>
      <button type="button" class="btn-primary" data-jump-week="${latestKey}">
        Go to week with hours (${formatDisplayDate(labelStart)} – ${formatDisplayDate(labelEnd)})
      </button>
    </div>
  `;
}

function weekEntriesHaveHours(weekStart, weekEnd) {
  return state.entries.some(
    (e) =>
      entryOverlapsDateRange(e, weekStart, weekEnd) &&
      (!isEmployeeUser() || e.employeeId === currentEmployeeId()) &&
      entryMatchesWeekJobFilter(e)
  );
}
...
function jobGroupMarkup(members) {
  ...
  const estValues = members.map((m) => Number(m.estimatedHours) || 0);
  const nbValues = members.map((m) => Number(m.nonBillableHours) || 0);
  const estMin = Math.min(...estValues);
  const estMax = Math.max(...estValues);
  const nbMin = Math.min(...nbValues);
  const nbMax = Math.max(...nbValues);
  const estVary = !ld && !nbOnly && estMin !== estMax;
  const nbVary = nbMax > 0 && nbMin !== nbMax;

  const hoursBit = ld
    ? `<span class="badge badge-ld">LD</span>`
    : nbOnly
      ? `<span class="badge badge-nb">NB only</span> ${
          nbVary
            ? `${formatHours(nbMin)}–${formatHours(nbMax)} hrs`
            : `${formatHours(entry.nonBillableHours)} hrs`
        }`
      : estVary
        ? `${formatHours(estMin)}–${formatHours(estMax)} est.`
        : `${formatHours(entry.estimatedHours)} est.`;
  const nb =
    !nbOnly && nbMax > 0
      ? `<span class="badge">+${
          nbVary
            ? `${formatHours(nbMin)}–${formatHours(nbMax)}`
            : formatHours(nbMax)
        } NB</span>`
      : "";
  const noteBits = [];
...
function formatTruckCrewLines(members) {
  ...
  const hoursSuffix = (m) => {
    const bits = [];
    if (!isLdEntry(m) && !isNbOnlyEntry(m) && Number(m.estimatedHours) > 0) {
      bits.push(`${formatHours(m.estimatedHours)} est`);
    }
    if (isNbOnlyEntry(m) || Number(m.nonBillableHours) > 0) {
      bits.push(`${formatHours(m.nonBillableHours)} NB`);
    }
    if (isLdEntry(m) && !bits.length) bits.push("LD");
    return bits.length ? ` · ${bits.join(" · ")}` : "";
  };

  if (isNbOnlyEntry(members[0]) || Number(members[0].truckCount) === 0) {
    return members
      .map((m) => {
        const role = m.jobRole || employeeRole(m.employeeId);
        return `${escapeHtml(employeeName(m.employeeId))} <span class="role-badge role-${role === "driver" ? "driver" : "mover"}">${roleLabel(role)}</span>${hoursSuffix(m)}`;
      })
      .join(", ");
  }
  ...
  if (!hasTruckData) {
    return members
      .map((m) => {
        ...
        return `${escapeHtml(employeeName(m.employeeId))} <span class="role-badge role-${role}">${roleLabel(role)}</span>${hoursSuffix(m)}`;
      })
      .join(", ");
  }
  ...
  return [...byTruck.keys()]
    ...
    .map((num) => {
      const bucket = byTruck.get(num);
      const driverBit = bucket.driver
        ? `${escapeHtml(employeeName(bucket.driver.employeeId))} <span class="role-badge role-driver">Driver</span>${hoursSuffix(bucket.driver)}`
        : "No driver";
      const movers = bucket.movers
        .map(
          (m) =>
            `${escapeHtml(employeeName(m.employeeId))}${hoursSuffix(m)}`
        )
        .join(", ");
      return `<div class="truck-line"><strong>Truck ${num}:</strong> ${driverBit}${
        movers ? ` · ${movers}` : ""
      }</div>`;
    })
    .join("");
